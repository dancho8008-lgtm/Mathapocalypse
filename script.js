...
function playTone(type, frequency, duration, volume, slideTo = null) {
  ...
  gain.gain.exponentialRampToValueAtTime(volume * settings.masterVolume, now + 0.02);
...
function playSoundEffect(name) {
  ...
  audio.preload = "auto";
  audio.volume = config.volume * settings.masterVolume;
...
function applySettingsFromForm() {
  ...
  if (settings.singleDigitOnly && getSingleDigitMultiplicationProblems().length === 0) {
  ...
  settings.masterVolume = clamp((Number(volumeInput.value) || 75) / 100, 0, 1);
